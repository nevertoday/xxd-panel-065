...
# 🦁 XXD Panel 065｜투 라인 에디토리얼 저널

### 검정색 구조선과 두 개의 소스 이미지 색상 선을 사용하여 손으로 그린 색상 리듬 만들기
...
일반적인 선화는 윤곽을 설명할 수 있지만 인쇄물 특유의 리듬이 부족할 때가 많습니다. 반대로 색 선을 무작정 겹치면 사진과 무관한 색이 어지러운 잔상으로 남기 쉽습니다. 진짜 어려운 점은 구조와 손맛, 제한된 색이 만드는 긴장감을 동시에 지키는 것입니다.

**Panel 065**는 가느다란 검정 선으로 정확한 골격을 세우고, 원본 이미지에서 서로 긴장감을 만드는 두 색을 골라 색 선으로 사용합니다. 미세한 어긋남과 겹침, 나란한 선으로 수작업 다색 인쇄와 오래된 인쇄의 오차를 재현해 사진을 절제되고 알아보기 쉬우며 실제 손맛이 느껴지는 편집 선화로 바꿉니다.

### 이런 상황에 적합합니다

- **복잡한 구조를 선으로 지켜야 할 때:** 인물의 자세, 건축의 방향, 사물의 비례와 대상 간 관계가 한눈에 읽혀야 합니다.
- **다채로운 잔상이 아니라 색판 인쇄의 리듬을 원할 때:** 검정 선은 구조를 맡고, 원본에서 가져온 두 색의 선은 냉온 또는 명암의 긴장감을 만듭니다.
- **오래된 인쇄의 불완전함을 좋아할 때:** 어긋남, 끊긴 선, 선을 넘은 자국과 수정 흔적이 고장 난 듯한 효과가 아니라 자연스럽고 절제되게 보여야 합니다.
...
> XXD Panel 065으로 이 이미지를 작업하고, 먼저 가장 어울리는 구성과 크기를 추천해 줘.
...
**키워드:** 검정색 구조선 · 2색 강조선 · 손으로 칠한 색상 등록 · 오래된 인쇄 위치 변경 · 미세 조판

Panel 065는 손으로 그린 가늘고 간헐적으로 보정 흔적이 있는 검은색 선을 뼈대로 사용한 다음 사진에서 두 가지 긴장된 강조 색상을 선택하여 컬러 선과 검은색 선을 약간 어긋나거나 겹치거나 평행하게 만듭니다.

<!-- xxd-panel-benefit:start -->
## 빠른 판단: XXD Panel 065이 나에게 맞을까요?

| 궁금한 점 | 이 스타일이 주는 답 |
|---|---|
| **얻게 되는 결과** | 검정색 구조선과 두 개의 소스 이미지 색상 선을 사용하여 손으로 그린 색상 리듬 만들기 |
| **한눈에 알아볼 특징** | 검정색 구조선 · 2색 강조선 · 손으로 칠한 색상 등록 · 오래된 인쇄 위치 변경 · 미세 조판 |
...
정확한 검정색 구조 드로잉은 자세, 방향 및 관계를 유지한 다음 유용한 장력을 지닌 두 가지 소스 색상이 정합선이 됩니다. 약간의 오정렬, 겹침 및 평행 획으로 인해 결과는 절제되고 진정으로 인쇄된 사설 시트처럼 느껴집니다.

```text
주요 구조, 자세, 방향 파악 → 가는 검은색 선으로 뼈대 설정 → 시각적 긴장감을 가지고 두 가지 원본 색상 선택 → 색상 선을 잘못 등록하거나 겹치거나 평행하게 → 크기와 공간을 통해 밀도 조절 → 좁은 마이크로 타이포그래피 통합
```

## 완성 스타일을 알아보는 특징

- 검정색 손으로 그린 선은 흔들림, 일시 중지, 오버슈트, 중단 및 수정을 유지하는 구조적 뼈대입니다.
- 두 개의 액센트 라인은 소스에서 나오며, 추가 색상 없이 따뜻한-차가운 또는 어두운-밝은 긴장감을 형성하는 것이 좋습니다.
- 색상 선은 레지스터에서 약간 벗어나거나 겹치거나 검정색 옆에 배치되어 수제 등록 및 오래된 인쇄 리듬을 만듭니다.
- 넓은 색상 채우기가 없습니다. 선, 규모, 방향, 여백이 시각적 무게감을 만들어낸다.
- 좁은 활판 인쇄와 같은 마이크로타이프는 약간의 기본 오류를 수반하며 그림의 솔직한 불완전성을 공유합니다.

미적 제약과 금지 항목은 [중국어 원본 프롬프트](references/original-prompt/zh-CN.md)에만 있습니다. Skill과 런타임 어댑터는 이번 전달 변수만 처리합니다. [Skill 워크플로](SKILL.md) · [영문 런타임 어댑터](references/xxd-panel-065-prompt.en.md)
...
이미지 폴더를 직접 전달하면 이 병사 Skill이 같은 Panel 065의 원본 미학으로 일괄 처리합니다. 일반 래스터 형식을 기본적으로 재귀 탐색하고 수량을 먼저 알린 뒤 모드·크기·텍스트·언어를 한 번만 확인합니다. 각 이미지는 내용이나 문구가 섞이지 않도록 독립 생성하며 전체 배치는 하나의 평면 작업 디렉터리만 사용합니다. 개별 실패는 기록하고 이후 이미지를 조용히 누락하지 않습니다.
...
/xxd-panel-065 photo.jpg --mode top-bottom --size auto --text prompt --locale ko-KR

# 좌우 비교: 3:2 가로형, 글자 없음
/xxd-panel-065 photo.jpg --mode left-right --size 3:2 --text none

# 디자인 전용: 9:16 휴대폰 세로형, 한국어 문구
/xxd-panel-065 photo.jpg --mode design-only --size 9:16 --text prompt --locale ko-KR
...
/xxd-panel-065 photo.jpg --mode design-only --size 3:4 --text prompt --locale ko-KR

# 정확한 문구를 재작성·번역·제목 추가 없이 사용
/xxd-panel-065 photo.jpg --mode design-only --size 3:4 --copy "빛을 이곳에 남겨 두기" --locale ko-KR

# 2개 모드 × 3개 크기로 독립적인 완성 구도 6개 생성
/xxd-panel-065 photo.jpg --mode top-bottom,design-only --size 1:1,3:4,9:16 --text prompt --locale ko-KR

# 폴더 전체에 Panel 065 적용, 공통 설정은 한 번만 확정
/xxd-panel-065 "/path/to/photos" --mode design-only --size auto,9:16 --text prompt --locale ko-KR
...
Claude Code 사용자는 같은 폴더를 다음 위치에 연결할 수 있습니다: `~/.claude/skills/xxd-panel-065`. 설치 후 에이전트 세션을 다시 시작하세요.
...
전체 사양: [Skill 워크플로](SKILL.md) · [원본 스타일 자료](references/original-prompt/zh-CN.md) · [영문 런타임 어댑터](references/xxd-panel-065-prompt.en.md) · [중문 런타임 어댑터](references/xxd-panel-065-prompt.zh-CN.md)
...
| **[xxd-panel-065](https://github.com/nevertoday/xxd-panel-065)** | 검은 주도형 · 거대한 여백 · 망점 소멸 · 선적 사유 · 생각의 파편 |
...
| **병사급** | `xxd-panel-NNN` (현재 001–065) | 각 번호가 고유한 원본 프롬프트와 미학만 실행해 장군이 배정한 하나의 작업을 완성합니다. |
