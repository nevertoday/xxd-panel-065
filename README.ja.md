...
# 🦁 XXD Panel 065｜二本線エディトリアル手帳

### 黒の構造線と 2 つのソース画像の色の線を使用して、手描きの色のリズムを作成します
...
一般的な線画は輪郭を伝えられても、印刷物らしいリズムが不足しがちです。かといって色線を無造作に重ねると、写真とは無関係な色が散らかった残像になってしまいます。本当に難しいのは、構造、手触り、限られた色数が生む緊張感を同時に保つことです。

**Panel 065** は、細い黒線で正確な骨格を作り、原画像から互いに緊張感のある 2 色を選んで色線にします。わずかな版ずれ、重なり、並走する線で手作業の多色刷りや古い印刷の誤差を再現し、写真を抑制が利き、判別しやすく、確かな手触りのあるエディトリアル線画へ変えます。

### このような場面に適しています

- **複雑な構造を線で保ちたい：** 人物の姿勢、建築の方向、器物の比率、対象同士の関係が一目で分かる必要がある。
- **多色の残像ではなく、版ずれのリズムが欲しい：** 黒線が構造を担い、原画像由来の 2 色の線が寒暖または明暗の緊張感を生みます。
- **古い印刷の不完全さが好き：** ずれ、途切れ、はみ出し、修正の跡を、故障風のエフェクトではなく自然かつ控えめに見せたい。
...
> XXD Panel 065 でこの画像を処理して、まず最適な構図とサイズを提案して。
...
**キーワード:** 黒の構造線・2色のアクセント線・手描きの色登録・古い印刷のずれ・マイクロ植字

Panel 065は、修正跡のある細く断続的な黒い手描きの線を骨格として、写真の中から緊張感のあるアクセントカラーを2色選び、色の線と黒い線を微妙にずらしたり、重ねたり、平行させたりしています。

<!-- xxd-panel-benefit:start -->
## すぐに判断：XXD Panel 065 はあなたに合う？

| 知りたいこと | このスタイルの答え |
|---|---|
| **得られる完成作品** | 黒の構造線と 2 つのソース画像の色の線を使用して、手描きの色のリズムを作成します |
| **ひと目で分かる特徴** | 黒の構造線・2色のアクセント線・手描きの色登録・古い印刷のずれ・マイクロ植字 |
...
正確な黒の構造図は姿勢、方向、関係を維持し、有用な張力を持つ 2 つのソース カラーが位置合わせ線になります。わずかな位置のずれ、重なり、平行なストロークにより、結果は抑制された、本物に印刷されたエディトリアル シートのように感じられます。

```text
キーの構造、姿勢、方向を特定する → 細い黒い線で骨格を確立する → 視覚的な緊張感を持って 2 つのソースカラーを選択する → 色付きの線を位置合わせする、重ねる、または平行にする → スケールとスペースによって密度を制御する → 細いマイクロタイポグラフィを統合する
```

## 完成作品を見分けるポイント

- 黒い手描きの線は構造的な骨格であり、ぐらつき、一時停止、オーバーシュート、中断、修正を保持します。
- 2 つのアクセント ラインは厳密にソースから来ており、できれば余分な色を使用せずに、暖色と寒色、または暗色と光の緊張感を形成します。
- カラーラインは、レジストレーションからわずかにずれたり、重なり合ったり、黒の横を走ったりして、手作りのレジストレーションと古い印刷物のリズムを作り出します。
- 広い色の塗りつぶしはありません。線、スケール、方向、空きスペースが視覚的な重みを作成します。
- 幅の狭い活版印刷のようなマイクロタイプには、わずかなベースライン エラーが含まれており、図面の正直な不完全さを共有しています。

美的制約と避けるべき表現は[中国語の原文プロンプト](references/original-prompt/zh-CN.md)だけにあります。Skill と実行アダプターは今回の納品条件だけを扱います。 [Skill ワークフロー](SKILL.md) · [英語ランタイムアダプター](references/xxd-panel-065-prompt.en.md)
...
画像フォルダーを直接渡すと、この兵士 Skill が同じ Panel 065 の原文美学で一括処理します。一般的なラスター形式を既定で再帰検索し、件数を報告してから、モード・サイズ・文字・言語を一度だけ確認します。各画像は内容や文言を混ぜず独立生成し、バッチ全体は一つのフラットなタスクフォルダーへ保存します。個別の失敗は記録し、後続画像を黙って省略しません。
...
/xxd-panel-065 photo.jpg --mode top-bottom --size auto --text prompt --locale ja-JP
...
/xxd-panel-065 photo.jpg --mode design-only --size 3:4 --text prompt --locale ja-JP

# 正確な文言を、書き換え・翻訳・タイトル追加なしで使用
/xxd-panel-065 photo.jpg --mode design-only --size 3:4 --copy "光をここに残す" --locale ja-JP

# 2モード × 3サイズで独立した完成構図を6枚生成
/xxd-panel-065 photo.jpg --mode top-bottom,design-only --size 1:1,3:4,9:16 --text prompt --locale ja-JP

# フォルダー全体へ Panel 065 を適用し、共通設定は一度だけ解決
/xxd-panel-065 "/path/to/photos" --mode design-only --size auto,9:16 --text prompt --locale ja-JP
...
Claude Code では同じフォルダを次へリンクできます： `~/.claude/skills/xxd-panel-065`. インストール後に Agent セッションを再起動してください。
...
完全仕様: [Skill ワークフロー](SKILL.md) · [原始スタイル資料](references/original-prompt/zh-CN.md) · [英語ランタイムアダプター](references/xxd-panel-065-prompt.en.md) · [中国語ランタイムアダプター](references/xxd-panel-065-prompt.zh-CN.md)
...
| **[xxd-panel-065](https://github.com/nevertoday/xxd-panel-065)** | 黒い主形 · 巨大な余白 · 網点の消散 · 禅的思考 · 思考の断片 |
...
| **兵士級** | `xxd-panel-NNN`（現在 001–065） | 各番号が固有の原文プロンプトと美学だけを実行し、将軍から渡された一つの仕事を完成させます。 |
