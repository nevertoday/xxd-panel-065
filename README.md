...
**Panel 065** 适合这类任务：需要保留原图核心内容，同时希望最终视觉达到以下方向：**用黑色结构线与两种源图彩线制造手绘套色节奏**。它先从用途与期望结果出发，再决定构图、尺寸、文字和具体输出形式。
...
以准确的黑色手绘结构线保住照片的姿态、方向和关系，再从原图选出两种彼此有张力的颜色作为套印线。轻微错位、叠压和并行让画面像一页真实而克制的旧印刷编辑稿。

```text
识别关键结构、姿态与方向 → 用纤细黑线建立骨架 → 选择两种有张力的源图颜色 → 让彩线错位、叠压或并行 → 以留白和尺度控制疏密 → 加入窄体微型排印
```

## 成品中最容易识别的风格特点

- 黑色手绘线是结构骨架，保留抖动、停顿、越线、断线与修正。
- 两种强调线严格来自原图，优先形成冷暖或深浅关系，不增加杂色。
- 彩线与黑线轻微错位、叠压或并行，呈现手工套色和旧印刷节奏。
- 不做大面积填色；主体靠线条、尺度、方向和空白建立视觉重量。
- 窄体铅字与机械印刷感小字略有基线偏差，与线稿共享真实的不完美。

原始审美约束与拒绝项只存在于[原始提示词](references/original-prompt/zh-CN.md)；Skill 与运行适配器只处理本次交付变量。 [Skill 工作流](SKILL.md) · [英文运行适配器](references/xxd-panel-065-prompt.en.md)
...
| **[xxd-panel-065](https://github.com/nevertoday/xxd-panel-065)** | 黑色主形 · 极大留白 · 网点消散 · 禅意思考 · 哲思碎片 |
...
| **士兵级** | `xxd-panel-NNN`（当前 001–065） | 每个编号只执行自己独立的原始提示词与审美，把将军派发的单个任务完成为成品。 |
